...
//include('Auth.php');
	...
	function PIFEmail($nlvl,$nMax,$eMail, $cIDno, $EmpNamex, $cControlNox)
		{
			$cApprover1="";
	        $eMail="";
	        $fApprover = "Select vAppName from tblapprovers where cControlNumber = '" . $cControlNox . "' and nLevel = '1'";
			...
			$message = "You have a pending PIF for approval. Please see below details: <br>";
			...
			$message .= '<a href="http://192.168.1.236/guessemployeeonline/payroll/rankandfile/emailresponse.php?x=APPROVED&cSysIdx=' . trim($cControlNox) . '&nLvl=' . trim($nlvl) . '&xLvlMax=' . trim($nMax) . '" target="_blank" style="background-color: #999999;color: white; border: 1px solid #000000">APPROVE</a> 
						 <a href="http://192.168.1.236/guessemployeeonline/payroll/rankandfile/emailresponse.php?x=REJECTED&cSysIdx=' . trim($cControlNox) . '&nLvl=' . trim($nlvl) . '&xLvlMax=' . trim($nMax) . '" target="_blank" style="background-color: #999999;color: white; border: 1px solid #000000">REJECT</a><br>';
			...
			$headers .= "BCC: mabrodriguez@guess.com.ph;" . "\r\n";
		...
		if ($maxi>0)
				{
						...
						//check approver max lvl
							$query = "SELECT max(nLevel) as 'nMax' from tblapprovers where cControlNumber = '" .  $cControlNo . "'"; 
							$result = mysql_query($query) or die(mysql_error());
							while ($rowAxz = mysql_fetch_array($result))  
								{
									$nMaxz = $rowAxz['nMax'];
								}

						// trigger email function
						PIFEmail(1,$nMaxz,$eMail, $cIDNumber, $vName, $cControlNo);

						//this line insert into header table tblpayroll
						$qryAdd ="insert into tblpayroll (cControlNumber,cIDNumber,vName,vDepartment,vDesignation,cApprover1,cApprover2,dRequestDate,vAgency,cCurrentLocation) ";
						$qryAdd = $qryAdd . "Values (";
						$qryAdd = $qryAdd . "'" . $cControlNo . "',";
