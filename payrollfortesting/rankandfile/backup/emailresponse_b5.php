<?php 
    ...
    $lvl = $lvl + 1;
            ...
            function PIFEmail($nlvl, $eMail, $cControlNox)
                {
                 ...
                 $to = 'jpsarinas@guess.com.ph';
                    ...
                    $message .= '<a href="http://192.168.1.236/guessemployeeonline/payrollfortesting/rankandfile/emailresponse.php?x=APPROVED&cSysIdx=' . trim($cControlNox) . '&xLvl=' . trim($nlvl) . '" target="_blank" style="background-color: #999999;color: white; border: 1px solid #000000">APPROVE</a> 
                                <a href="http://192.168.1.236/guessemployeeonline/payrollfortesting/rankandfile/emailresponse.php?x=REJECTED&cSysIdx=' . trim($cControlNox) . '&xLvl=' . trim($nlvl) . '" target="_blank" style="background-color: #999999;color: white; border: 1px solid #000000">REJECT</a><br>';
            ...
            /* Email Function End */
             ...
             $isResponded="";
             $fApprover = "Select cStatus from tblapprovers where cControlNumber = '" . $cSysIdz . "' and nLevel = '". $lvl ."'";
            ...
            if($isResponded == 'APPROVED'){
                ...
                //get max approver
                $max=0;
                $fApprover = "Select max(nLevel) as 'maxLvl' from tblapprovers where cControlNumber = '" . $cSysIdz . "'";
                $rApprover = mysql_query($fApprover) or die(mysql_error());
                $exists=mysql_num_rows($rApprover);
                if ($exists>0)
                    {
                ...
                {   
                    ...
                    $qryAdd = "update tblpayroll set cApprover1Status='APPROVED', cApprover1='" . $dApproveDateTime . "',
                                cCurrentLocation='Payroll for Processing' where cControlNumber = '" . $cSysIdz . "'";
                    $qryAddResult=mysql_query($qryAdd);     
                    
                    $qryAdd = "update tblpayrolldetails set dPayrollRemarks='No Remarks yet',dPayrollAction='No Remarks yet',
                                dAddResponse='No Remarks yet'  where cControlNumber = '" . $cSysIdz. "'";
                    $qryAddResult=mysql_query($qryAdd);   
                    ...
                    PIFEmail($lvl,'mabrodriguez@gues.com.ph' ,$cSysIdz);
