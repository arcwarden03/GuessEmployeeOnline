<?php
    ...
    if($wBase=='HO'){
        ...
        if ($rowx === true){
            ...
            $pend = 'PENDING';
            $arc = '1900-01-01';
            $insQry = "INSERT into tOTQue (cSysID, cIDNumber, vName, cWorkBase, vDeptStore, dFiledBy, dFiledDate, dDate, cDayOff, 
                                        isDayOff, cShift, dFrom, dTo, vReason, vOtherR, cStatus, dArchiveDate) 
                                        values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $insParam = array($cControlNox, $cIDNumber, $vFName, $wBase, $vDepartment, $vName, $dNow, 
            $dDate, $zero, $zero, $zero, $dFrom, $dTo, $tReason, $tOtherR, $pend, $arc);
