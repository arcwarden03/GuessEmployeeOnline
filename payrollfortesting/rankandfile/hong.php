      ...
      <div class="col-md-6" id="dView">
        <div class="box box-primary with-border">
          ...
          <div class="box-body">
             ...
             <form method="POST" action="submit2.php" role="form">
             <?php
              ...
              <table class="table table-bordered table-hover"> 
                  ...
                  while($row = mysqli_fetch_assoc($result)){
                   ...
                   // ";
                    ...
                    echo"<td><a href='hong.php?id=$row[id]'><button class='btn btn-danger'>Remove</button></a></td>";
          ...
          $nLevel = 0;
            while ($cnt<=5)
            {
              if(isset($_POST['App' . $cnt]))
              {
                ...
                {
                  $nLevel = $nLevel + 1;
                  $qryAdd ="insert into tblapprovers (vAppName, nLevel) ";
...
<?php 
 include('DBConnect.php');
  if(isset($_POST['submit'])){
    ...
    $dDate = mysqli_real_escape_string($conn,$_POST['dDate']);
     $dReason = mysqli_real_escape_string($conn,$_POST['dReason']);
     ...
     $csysqry = "Select case when right(rtrim(cControlNumber),3) is null then 0 else right(rtrim(cControlNumber),3) end as cControlNumber from ";
        ...
        $qrycControlNumber=mysqli_query($conn, $cControlNumber);
            if (mysqli_num_rows($qrycControlNumber))
                {
                    $row = mysqli_fetch_array($qrycControlNumber,MYSQLI_NUM);
                ...
                $cControlNumber= $cIDNumber . "-" . str_pad($NewLastId + 1, 3, '0', STR_PAD_LEFT);




    $insert_query = "INSERT INTO payrolltemptable (cControlNumber,cIDNumber,vName,vDepartment,vDesignation,dDate,dMonth,dCutoff,dYear,dReason,vApprover,vApproverStatus) VALUES ('$cControlNumber','$cIDNumber','$vName','$vDepartment','$vDesignation','$dDate','$dMonth','$dCutoff','$dYear','$dReason','N/A','PENDING')";
    
    if(mysqli_query($conn, $insert_query)) { ?>
      <script>window.location = 'hong.php'</script>
...
<?php 
  ...
  if(isset($_POST['submit2'])){
    ...
    $insert_query = "INSERT INTO tblpayroll (id,cControlNumber,cIDNumber,vName,vDepartment,vDesignation,vApprover,vApproverStatus)";
     $insert_query = $insert_query  . "select id,cControlNumber,cIDNumber,vName,vDepartment,vDesignation,vApprover,vApproverStatus from payrolltemptable where cIDNumber = '" . $cIDNumber . "'";
      echo $insert_query ;
      $qryinsert_query=mysqli_query($insert_query);
  ...
  if(isset($_POST['submit2'])) {
            ...
            $insert_query2  ="INSERT INTO tblpayrolldetails (cControlNumber,cIDNumber,dDate,dMonth,dCutoff,dYear) ";
            $insert_query2 = $insert_query2  . "select cControlNumber,cIDNumber,dDate,dMonth,dCutoff,dYear from payrolltemptable where cIDNumber = '" . $cIDNumber . "'";
            echo $insert_query2 ;
            $qryinsert_query2=mysqli_query($insert_query2);
 ...
 if(isset($_POST['submit2'])) {
          ...
          $insert_query3 = "DELETE FROM payrolltemptable WHERE cIDNumber = '" . $cIDNumber . "'";
            echo $insert_query3 ;
            $qryinsert_query3=mysqli_query($insert_query3);
