...
function OTEmail($nlvl, $nMax, $eMail, $cControlNo)
       {
            ...
            include ('../../config/dbconfig-OT2.php');
            $queryx = "SELECT * from tOTProcess where cSysID = '" . $cControlNo . "' and cStatus <> 'APPROVED' order by cSysID asc";  
        ...
        if($lvlmax == $lvl){
            ...
            if($actionx == "APPROVED")
            {
                ...
                if(trim($_SESSION['SESS_vDesignation'])=='Sales Operation Manager'){
                    ...
                    session_write_close();
                    header("location: ../approver/view/OTApprovalHO_SOM.php");    
                } else {
                    $_SESSION['U_ValidMessage'] = 'OT Application Successfully Approved!';
                    session_write_close();
            ...
            else
            {
                ...
                $dApproveDateTimex=date("y/m/d : H:i:s", time()); 
                $upStatAP = "update tApprover set cStatus = '" . $actionx . "', 
                dApproveDate = '" . $dApproveDateTimex . "' 
                where cSysID = '" . $cSysIdz . "' and nLevel= '" . $lvl ."'";
                $upStatAPRes = sqlsrv_query($conn, $upStatAP);

                $upStatAPz = "update tOTProcess set cStatus = '" . $actionx . "'
                            where cSysID = '" . $cSysIdz . "'";
                $upStatAPResz = sqlsrv_query($conn, $upStatAPz);
