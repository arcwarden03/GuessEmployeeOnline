	...
	function PIFEmail($nlvl,$nMax, $eMail, $cIDno, $EmpNamex, $cControlNox)
		{
			...
			$message .= '<a href="http://192.168.1.236/guessemployeeonline/payrollfortesting/rankandfile/emailresponse.php?x=APPROVED&cSysIdx=' . trim($cControlNox) . '&nLvl=' . trim($xlvl) .' &xLvlMax=' . trim($nMax) . '" target="_blank" style="background-color: #999999;color: white; border: 1px solid #000000">APPROVE</a> 
						 <a href="http://192.168.1.236/guessemployeeonline/payrollfortesting/rankandfile/emailresponse.php?x=REJECTED&cSysIdx=' . trim($cControlNox) . '&nLvl=' . trim($xlvl) . '" target="_blank" style="background-color: #999999;color: white; border: 1px solid #000000">REJECT</a><br>';
		...
		if ($maxi>0)
				{
						...
						PIFEmail(1,$nMaxz,'mabrodriguez@guess.com.ph', $cIDNumber, $vName, $cControlNo);

						         //check approver max lvl
             $qryApprover2 = "SELECT max(nLevel) as 'nMax' from tblapprovers where cControlNumber = '" .  $cSysIdz . "'"; 
            $rslt2 = mysql_query($conn, $qryApprover2);
            while ($rowAbc = mysql_fetch_array($rslt2))  
             {
             $nMaxz = trim($rowAbc['nMax']);
             }
