<?php 
    ...
    function OTEmail($nlvl, $eMail, $cIDNumberx)
    {
            $dNow = date('Y/m/d');
            include ('../../../config/dbconfig-OT2.php');
            $query = "SELECT * from tOTProcess where cidnumber = '" . $cIDNumberx . "'  and dFiledDate like '%". $dNow ."%' order by cSysID asc";  
            ...
            $message .= '<a href="http://192.168.1.236/guessemployeeonline/overtime/rankandfile/model/emailresponse.php?x=APPROVED&cSysIdx=' . trim($cControlNox) . '&xLvl=' . trim($nlvl) . '" target="_blank" style="background-color: #999999;color: white; border: 1px solid #000000">APPROVE</a> 
                        <a href="http://192.168.1.236/guessemployeeonline/overtime/rankandfile/model/emailresponse.php?x=REJECTED&cSysIdx=' . trim($cControlNox) . '&xLvl=' . trim($nlvl) . '" target="_blank" style="background-color: #999999;color: white; border: 1px solid #000000">REJECT</a><br>';
    ...
    while ($rowAz = sqlsrv_fetch_array($rslt))  
    {
        ...
        $rsltAddDetailed=sqlsrv_query($conn, $qryAddDetailed);
            ...
            $query = "SELECT * from vAppDet where cIDNumber = '". $cIDNumber ."' order by nLevel asc"; 
            $rslt = sqlsrv_query($conn, $query);
    ...
    OTEmail(1,'jpsarinas@guess.com.ph',$cIDNumber);

    $_SESSION['U_ValidMessage'] = 'OT Application Successfully Submitted For Approval!';
    //$_SESSION['U_ValidMessage'] = $cSysIDx .' - '. $cIDNumber .' - '. $vAppNamex .' - '. $nLvlx .' - '. $vEmailx .' - '. $statx .' - '. $pendD;
    session_write_close();
